...
export function registerApplication(name: string, Component: React.FunctionComponent<any>) {
  // TODO - does it really make sense that every one of these gets the same redux store? Might we make that configurable?
  //        or maybe it belongs up a level in the app component being passed in. One argument for here, is if we ever want
  //        to share a single redux store across multiple embedded apps. One argument for up a level, is not every embedded
  //        app might need it, and not every one might need the same reducers.
